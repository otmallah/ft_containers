...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/15 15:35:56 by otmallah          #+#    #+#             */
/*   Updated: 2023/01/15 20:32:39 by otmallah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */


#include "../map/map.hpp"       // your Map path.
#include "../utils/make_pair.hpp"  // path to std::pair.
...
typedef std::pair<ft::map<int, std::string>::iterator, ft::map<int, std::string>::iterator> ft_iter_def;
typedef std::pair<std::map<int, std::string>::const_iterator, std::map<int, std::string>::const_iterator> const_iter_def;
typedef std::pair<ft::map<int, std::string>::const_iterator, ft::map<int, std::string>::const_iterator> ft_const_iter_def;
...
void testOperations()
{
              ...
              << "] --------------------]\t\t\033[0m";
    {
        ...
        /*---------------------------------- time limit test --------------------------------------------*/
        {
            ...
            start = get_time();
            res = m.lower_bound(1e3)->first;
            end = get_time();
            diff = end - start;
            diff = (diff) ? (diff * TIME_FAC) : TIME_FAC;

            ualarm(diff * 1e3, 0);
            ft_res = ft_m.lower_bound(1e3)->first;
            ualarm(0, 0);
            cond = ft_res == res;
        }
        std::map<int, std::string> m;
